...
#define MAC_BASE 1000
#define Auto_ASSIGNED_MAC -1

/*Store the subnet nodes mac information, type list*/
PrgT_List *Network_Mac_Info_List;
Boolean INITIAL_ONCE = OPC_FALSE;
...
static void networkMacInfoInit(void);
...
/*@param Void
Function: initializing the network and assign every node only-different mac address*/
static void networkMacInfoInit(void)
{
	...
	int mac_address;
	Node_Mac_Info* node_mac_info; 
	FIN(static void networkMacInfoInit(void));
	//node_total_number = op_topo_object_count (OPC_OBJTYPE_NODE_MOB);
	node_total_number = op_topo_object_count (NODE_TYPE);
	Network_Mac_Info_List = op_prg_list_create();
	for(node_index = 0; node_index < node_total_number; node_index++)
	{
		node_objid = op_topo_object (NODE_TYPE, node_index);
		if(op_ima_obj_attr_exists(node_objid,"Mac Address"))
			{
				node_mac_info = (Node_Mac_Info*) op_prg_mem_alloc(sizeof(Node_Mac_Info));
				node_mac_info->node_objid = node_objid;
				op_ima_obj_attr_get(node_objid,"Mac Address",&mac_address);
				//printf("Debug Info:--%d--", mac_address); 	 
				if(mac_address == Auto_ASSIGNED_MAC)
				{
					op_ima_obj_attr_set(node_objid,"Mac Address",(MAC_BASE + node_index));
					node_mac_info->mac_address = (MAC_BASE + node_index);
					prg_list_insert (Network_Mac_Info_List, node_mac_info, PRGC_LISTPOS_TAIL); 
					}
				else if( (mac_address !=-1) && (mac_address < 0) )
				{
					char node_name[128];
					char msg[128];
				...
				{
						node_mac_info->mac_address = mac_address ;
						prg_list_insert (Network_Mac_Info_List, node_mac_info, PRGC_LISTPOS_TAIL); 
					}
			}
	}
