/* Process model C form file: sink_measure_user.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char sink_measure_user_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 5855F515 5855F515 1 FlyingDutchman yzc 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1d79 5                                                                                                                                                                                                                                                                                                                                                                                                       ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	sink_measure_user_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((sink_measure_user_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void sink_measure_user (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_sink_measure_user_init (int * init_block_ptr);
	void _op_sink_measure_user_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_sink_measure_user_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_sink_measure_user_alloc (VosT_Obtype, int);
	void _op_sink_measure_user_svar (void *, const char *, void **);
...
void
sink_measure_user (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (sink_measure_user ());
		...
		FSM_ENTER_NO_VARS ("sink_measure_user")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (Move) enter executives **/
			FSM_STATE_ENTER_UNFORCED (0, "Move", state0_enter_exec, "sink_measure_user [Move enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (1,"sink_measure_user")


			/** state (Move) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "Move", "sink_measure_user [Move exit execs]")
				FSM_PROFILE_SECTION_IN ("sink_measure_user [Move exit execs]", state0_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (0, state0_enter_exec, ;, "default", "", "Move", "Move", " ", "sink_measure_user [Move -> Move : default / ]")
				/*---------------------------------------------------------*/



			/** state (INIT) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (1, "INIT", "sink_measure_user [INIT enter execs]")
				FSM_PROFILE_SECTION_IN ("sink_measure_user [INIT enter execs]", state1_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (1, "INIT", "sink_measure_user [INIT exit execs]")
				FSM_PROFILE_SECTION_IN ("sink_measure_user [INIT exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_FORCE (0, state0_enter_exec, ;, "default", "", "INIT", "Move", "tr_12", "sink_measure_user [INIT -> Move : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_sink_measure_user_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_sink_measure_user_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_sink_measure_user_terminate ())
...
/* local variable prs_ptr in _op_sink_measure_user_svar function. */
...
VosT_Obtype
_op_sink_measure_user_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_sink_measure_user_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (sink_measure_user)",
		sizeof (sink_measure_user_state));
	*init_block_ptr = 2;
...
VosT_Address
_op_sink_measure_user_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	sink_measure_user_state * ptr;
	FIN_MT (_op_sink_measure_user_alloc (obtype))

	ptr = (sink_measure_user_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "sink_measure_user [INIT enter execs]";
#endif
		}
...
void
_op_sink_measure_user_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	sink_measure_user_state		*prs_ptr;

	FIN_MT (_op_sink_measure_user_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (sink_measure_user_state *)gen_ptr;
