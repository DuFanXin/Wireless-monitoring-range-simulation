/* Process model C form file: measure_inter.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char measure_inter_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 56A0F20E 56A0F20E 1 lenovo-PC lenovo 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1d79 5                                                                                                                                                                                                                                                                                                                                                                                                         ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	measure_inter_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((measure_inter_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void measure_inter (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_measure_inter_init (int * init_block_ptr);
	void _op_measure_inter_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_measure_inter_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_measure_inter_alloc (VosT_Obtype, int);
	void _op_measure_inter_svar (void *, const char *, void **);
...
void
measure_inter (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (measure_inter ());
		...
		FSM_ENTER ("measure_inter")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (IDLE) enter executives **/
			FSM_STATE_ENTER_UNFORCED (0, "IDLE", state0_enter_exec, "measure_inter [IDLE enter execs]")
				FSM_PROFILE_SECTION_IN ("measure_inter [IDLE enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"measure_inter")


			/** state (IDLE) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "IDLE", "measure_inter [IDLE exit execs]")
				FSM_PROFILE_SECTION_IN ("measure_inter [IDLE exit execs]", state0_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("measure_inter [IDLE trans conditions]", state0_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "UPPER_STRM", "", "IDLE", "TRANSMIT", "tr_39", "measure_inter [IDLE -> TRANSMIT : UPPER_STRM / ]")
				FSM_CASE_TRANSIT (1, 0, state0_enter_exec, ;, "default", "", "IDLE", "IDLE", "tr_10", "measure_inter [IDLE -> IDLE : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (1, "TRANSMIT", state1_enter_exec, "measure_inter [TRANSMIT enter execs]")
				FSM_PROFILE_SECTION_IN ("measure_inter [TRANSMIT enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"measure_inter")


			/** state (TRANSMIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "TRANSMIT", "measure_inter [TRANSMIT exit execs]")


			/** state (TRANSMIT) transition processing **/
			FSM_PROFILE_SECTION_IN ("measure_inter [TRANSMIT trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 3, state3_enter_exec, radioTransimissionAreaDraw();, "SELF_INTRPT_Animation", "radioTransimissionAreaDraw()", "TRANSMIT", "Animation", "tr_41", "measure_inter [TRANSMIT -> Animation : SELF_INTRPT_Animation / radioTransimissionAreaDraw()]")
				FSM_CASE_TRANSIT (1, 0, state0_enter_exec, ;, "SELF_INTRPT_SEND", "", "TRANSMIT", "IDLE", "tr_42", "measure_inter [TRANSMIT -> IDLE : SELF_INTRPT_SEND / ]")
				FSM_CASE_TRANSIT (2, 1, state1_enter_exec, ;, "default", "", "TRANSMIT", "TRANSMIT", "tr_12", "measure_inter [TRANSMIT -> TRANSMIT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED_NOLABEL (2, "Init", "measure_inter [Init enter execs]")
				FSM_PROFILE_SECTION_IN ("measure_inter [Init enter execs]", state2_enter_exec)
				{
				/*获取节点和进程的id号*/
				own_id = op_id_self();
			...
			FSM_STATE_EXIT_FORCED (2, "Init", "measure_inter [Init exit execs]")
				FSM_PROFILE_SECTION_IN ("measure_inter [Init exit execs]", state2_exit_exec)
				{
				if(cluster_enable == OPC_TRUE)
					{
			...
			FSM_TRANSIT_FORCE (0, state0_enter_exec, ;, "default", "", "Init", "IDLE", "tr_38", "measure_inter [Init -> IDLE : default / ]")
				/*---------------------------------------------------------*/



			/** state (Animation) enter executives **/
			FSM_STATE_ENTER_UNFORCED (3, "Animation", state3_enter_exec, "measure_inter [Animation enter execs]")
				FSM_PROFILE_SECTION_IN ("measure_inter [Animation enter execs]", state3_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (7,"measure_inter")


			/** state (Animation) exit executives **/
			FSM_STATE_EXIT_UNFORCED (3, "Animation", "measure_inter [Animation exit execs]")
				FSM_PROFILE_SECTION_IN ("measure_inter [Animation exit execs]", state3_exit_exec)
				{
				if(SIM_END)
					{
			...
			FSM_TRANSIT_FORCE (3, state3_enter_exec, ;, "default", "", "Animation", "Animation", "tr_43", "measure_inter [Animation -> Animation : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_measure_inter_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_measure_inter_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_measure_inter_terminate ())
...
/* local variable prs_ptr in _op_measure_inter_svar function. */
#undef mac_id
#undef own_id
#undef node_id
...
VosT_Obtype
_op_measure_inter_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_measure_inter_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (measure_inter)",
		sizeof (measure_inter_state));
	*init_block_ptr = 4;
...
VosT_Address
_op_measure_inter_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	measure_inter_state * ptr;
	FIN_MT (_op_measure_inter_alloc (obtype))

	ptr = (measure_inter_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "measure_inter [Init enter execs]";
#endif
		}
...
void
_op_measure_inter_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	measure_inter_state		*prs_ptr;

	FIN_MT (_op_measure_inter_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (measure_inter_state *)gen_ptr;
